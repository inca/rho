...
var BlockCompiler
  = module.exports
  = function BlockCompiler(options, macros) {
  ...
  this.inline = new InlineCompiler(this.options, this.macros);
...
BlockCompiler.prototype = {
  ...
  emitSelector: function() {
    ...
    if (Array.isArray(this.selector.classes)) {
      ...
      for (var i in this.selector.classes) {
        if (i > 0) this.out += " ";
        this.out += this.selector.classes[i];
      }
      this.out += "\"";
    }
    // emit data-src
    if (this.options.sourceIndices)
      this.out += " data-src=\"" + this.blockSrc + "\"";
  },
  ...
  emitParagraph: function(walk) {
    ...
    if (walk.hasCurrent()) {
      ...
      this.out += "<p";
      this.emitSelector();
      this.out += ">" + this.inline.compile(p) + "</p>\n";
    }
  }
