...
var InlineCompiler
  ...
  this.macros = utils.merge(defaults.macros.inline, macros);
...
InlineCompiler.prototype = {
  ...
  */
  ...
  emitLink: function(text, link) {
    ...
    var html = new InlineCompiler(this.options, this.macros).compile(text);
    this.out += ">" + html + "</a>";
  },
  ...
  emitImg: function(alt, link) {
    ...
    if (!src)
      src = link.toString();
    this.out += "<img src=\"" + src + "\" alt=\"" + escapeHtml(alt) + "\"";
    if (link.title)
      this.out += " title=\"" + escapeHtml(link.title) + "\"";
    this.out += "/>";
  },
